...
public class GetPluginConfigurationExecutor implements RequestExecutor {
    ...
    public static final Field GO_SERVER_URL = new Field("go_server_url", "Go Server URL", null, true, false, "0") {
        @Override
    ...
    public static final Field DOCKER_URL = new Field("docker_uri", "Docker URI", null, true, false, "1") {
        @Override
        public String doValidate(String input) {
            if (StringUtils.isBlank(input)) {
                return this.displayName + " must not be blank.";
    ...
    public static final Field AUTOREGISTER_TIMEOUT = new Field("auto_register_timeout", "Agent auto-register Timeout (in minutes)", "10", true, false, "2") {
        @Override
        public String doValidate(String input) {
            try {
